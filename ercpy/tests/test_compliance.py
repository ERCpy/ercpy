...
class TestCaseCompliance(unittest.TestCase):
    """TestCase for checking the pep8 compliance of the ercpy package."""
    ...
    def test_pep8(self):
        ...
        files = self.get_files_to_check(os.path.join(self.path, 'ercpy')) \
            + self.get_files_to_check(os.path.join(self.path, 'scripts')) \
            + self.get_files_to_check(os.path.join(self.path, 'ercpy', 'tests'))
        ignores = ('E125', 'E226', 'E228')
        pep8.MAX_LINE_LENGTH = 99
        pep8style = pep8.StyleGuide(quiet=False)
        pep8style.options.ignore = ignores
        stdout_buffer = sys.stdout
        with open(os.path.join(self.path, 'ercpy', 'tests', 'pep8_log.txt'), 'w+') as sys.stdout:
            print '<<< PEP8 LOGFILE >>>'
